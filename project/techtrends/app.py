...
def get_post(post_id):
    ...
    # Log Message
    app.logger.info('Get a post {0} successfull'.format(post['id']))
    return post
...
def index():
    ...
    connection.close()
    # Log Message
    app.logger.info('Index page successfull')
...
def post(post_id):
    ...
    if post is None:
        # Log Message
        app.logger.info('Post not found - 404 error')
...
def create():
    # Log Message
    app.logger.info('Post creation started')
    ...
    if request.method == 'POST':
        ...
        if not title:
            ...
            # Log Message
            app.logger.info('Error: Title is required!')
        else:
            connection = get_db_connection()
            ...
            # Log Message
            app.logger.info('Post creation successfull')
